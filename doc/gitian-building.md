...
*Setup instructions for a Gitian build of darkWormcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the darkWormcoin
...
- [Building darkWormcoin Core](#building-darkWormcoin-core)
...
Clone the git repositories for darkWormcoin and Gitian.
...
in the darkWormcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building darkWormcoin Core
----------------

To build darkWormcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the darkWormcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/darkWormcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/darkWormcoin-project/darkWormcoin
...
For example:
```bash
URL=https://github.com/thrasher-/darkWormcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit darkWormcoin=${COMMIT} --url darkWormcoin=${URL} ../darkWormcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit darkWormcoin=${COMMIT} --url darkWormcoin=${URL} ../darkWormcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit darkWormcoin=${COMMIT} --url darkWormcoin=${URL} ../darkWormcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the darkWormcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../darkWormcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/darkWormcoin
SIGPATH=/some/root/path/darkWormcoin-detached-sigs

./bin/gbuild --url darkWormcoin=${BTCPATH},signature=${SIGPATH} ../darkWormcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/darkWormcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/darkWormcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/darkWormcoin-osx-build.assert
```
...
[darkWormcoin-project/gitian.sigs.DRWM](https://github.com/darkWormcoin-project/gitian.sigs.DRWM/) repository, or if that's not possible create a pull
