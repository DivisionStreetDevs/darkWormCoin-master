...
Run with the -testnet option to run with "play darkWormcoins" on the test network, if you
...
darkWormcoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- BitcoinMiner : Generates darkWormcoins (if wallet is enabled).
...
However, in open source software such as darkWormcoin Core, where everyone uses
...
pay attention to for reviewers of darkWormcoin Core code.

General darkWormcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: darkWormcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of darkWormcoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against darkWormcoin Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:darkWormcoin-project/darkWormcoin.git
